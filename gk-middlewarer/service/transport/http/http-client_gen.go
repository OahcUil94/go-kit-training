...
// NewClient creates a new StringService that will call methods at
...
func NewClient(addr string, wrappers ...ClientLayer) service.StringService {
	return NewClientWithConfig(addr, ClientConfig{ClientLayers: wrappers})
}

// NewClientWithOptions creates a new StringService that will call methods at
// the given address provided by the addr string.  This function takes a series
// of ClientLayer(s) that will be applied to the client before the
// subsequent method call.
func NewClientWithOptions(addr string, wrappers []ClientLayer, options []httptransport.ClientOption) service.StringService {
	return NewClientWithConfig(addr, ClientConfig{ClientLayers: wrappers, Options: options})
}

// NewClientWithConfig creates a new StringService that will call methods
...
// every Endpoint.
func NewClientWithConfig(addr string, config ClientConfig) service.StringService {
	if config.Method == "" {
		config.Method = "GET"
	}

	var (
		uppercaseEndpoint, _, _ = clientFactory(endpoint.PathUppercase, encodeUppercaseRequest, decodeUppercaseResponse, config)(addr)
		countEndpoint, _, _     = clientFactory(endpoint.PathCount, encodeCountRequest, decodeCountResponse, config)(addr)
	)
